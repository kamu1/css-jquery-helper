var Helper = {
	...
	tip:function(config, arg2,position){
		if (config == "clear"){		//传入clear清除所有弹层
			$(window.top.document).find("._fixed-dialog_._tip_").remove();
			return false;
		}
		var _content="";
		var _time = 2000;
		...
		var _direction = ["top","bottom","left","right","left-top","right-top","left-bottom","right-bottom"]
		//如果第二个参数包装位置，第二个参数忽略时间和类型
		if (_direction.indexOf(arg2)>=0){
			_state = typeof(position) == "string" ? position : "default";
			position = arg2;
		}
		...
		if ( typeof(position)!="string" || (_direction.indexOf(arg2)<0 && typeof(arg2) == "string")){
			position = "center";
		}
		...
		//未填位置时赋值
		position = typeof(position)=="string" ? "_" + position + "_" : "_top_";
		...
		if (_tip.length){
			...
			Helper.dialog({
				content:_content,
				opacity:0,
				zIndex: (config && config['zIndex']) ? config['zIndex'] : 0,
				overlayClose:true,
				overlay:{
					pointerEvents:"none",
					top:_top,
	...
	dialog: function(config, callback){
		if (config == "clear"){		//传入clear清除所有弹层
			$(window.top.document).find("._fixed-dialog_").remove();
			return false;
		}
		...
		//如果设置了zIndex，则写入到覆盖层中
		if (config && config["zIndex"]){
			_overlay['zIndex'] = config["zIndex"];
		}
		//覆盖层点击关闭
		...
		$(window.top.document.body).append('<div class="fixed-dialog _fixed-dialog_ cmn-middle" style="'+ (_opacity != "" ? 'background-color:'+ _opacity +';' : '') +'">\
		...
		//将弹窗最外层作赋值为this对象
		var _this=$(window.top.document.body).children("._fixed-dialog_:last");
		...
		if (_btns){
			...
			for (var btn in config.buttons){
				...
				if (_btn instanceof Function){
					...
					//快捷键
					if (typeof(_btn['hotKey']) == "string"){
						_obj.eq(k).attr("data-hot-key",_btn['hotKey']);
					}
	...
	hotKey:function(config){
		$(document).on("keydown",function(ev){
			...
			if (_target && _target.length){
				...
				//如果按键属于多字母热键,则停止keydown
				if (ev.key.length>1 && _keyAll.indexOf(ev.key.toLowerCase()) >= 0){
					return false;
				}
				//如果按键属于单字母,先判断有无功能热键 ,若有且排除自定义热键后,则停止keydown
				if (ev.key.length==1
					&& (ev.ctrlKey || ev.altKey || ev.shiftKey)
					&& _keyAll.indexOf(ev.key.toLowerCase()) >= 0){
					return false;
				}
			}
		});
